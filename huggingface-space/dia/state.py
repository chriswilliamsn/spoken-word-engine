...
class DecoderOutput:
    ...
    def get_current_tokens(self) -> torch.Tensor:
        """Get tokens at the current step for generation."""
        return self.generated_tokens[:, self.current_step:self.current_step+1, :]
        
    def get_generated_tokens(self) -> torch.Tensor:
        """Get all generated tokens up to current step."""
        return self.generated_tokens[:, :self.current_step, :]

    def append_tokens(self, tokens: torch.Tensor) -> bool:
        """Append new tokens and advance step. Returns False if all sequences ended."""
        self.generated_tokens[:, self.current_step, :] = tokens
        self.current_step += 1
        
        # Check if we should continue (not all sequences have ended)
        # For now, just check if we haven't exceeded max length
        return self.current_step < self.generated_tokens.shape[1]
    ...
    def prefill(self, dec_out: torch.Tensor, prefill_steps: list[int]):
        ...
        self.current_step = max(prefill_steps)  # Set current step to after prefill